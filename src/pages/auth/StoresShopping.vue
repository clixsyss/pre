    ...
    <template v-if="shouldShowOrderDetailsModal">
      <div class="modal-overlay" @click="closeOrderModal">
        ...
        <div class="modal-body">
          ...
          <div v-if="selectedOrder.status === 'cancelled' && selectedOrder.cancellationReason" class="info-section">
            <h3 class="section-title">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            ...
            <div class="info-card">
              <div class="info-item">
                <span class="info-label">{{ $t('reason') }}</span>
                <span class="info-value">{{ getCancellationReasonName(selectedOrder.cancellationReason) }}</span>
              </div>
              <div class="info-item" v-if="selectedOrder.customCancellationReason">
                <span class="info-label">{{ $t('customReason') }}</span>
                <span class="info-value">{{ selectedOrder.customCancellationReason }}</span>
              </div>
            </div>
          </div>
...
const fetchStores = async () => {
  ...
  try {
    ...
    try {
      ...
      storesData = await getStoresByProject(projectStore.selectedProject.id, { limit: 100 })
      console.log('✅ StoresShopping: Retrieved stores from DynamoDB:', storesData.length)
    } catch (dynamoError) {
    ...
    // Fetch ratings for stores (if not already included from DynamoDB)
    const storesWithRatings = await Promise.all(
      storesData.map(async (storeData) => {
        // If rating and reviewCount are already set from DynamoDB, use them
        if (storeData.rating !== undefined && storeData.reviewCount !== undefined) {
          return storeData
        }
        
        // Otherwise, fetch ratings from Firestore
        try {
          const ratingsPath = `projects/${projectStore.selectedProject.id}/ratings`;
          const ratingsQueryOptions = {
            filters: [
              { field: 'storeId', operator: '==', value: storeData.id }
            ],
            timeoutMs: 6000
          };
          ...
          console.error('Error fetching ratings for store:', storeData.id, ratingError);
          return {
            ...storeData,
...
const fetchUserOrders = async () => {
  ...
  try {
    ...
    let orders = []
    try {
      const { getUserOrders } = await import('src/services/dynamoDBOrdersService')
      orders = await getUserOrders(projectId, user.uid, { limit: 100 })
      console.log('Orders fetched from DynamoDB:', orders.length)
    } catch (dynamoError) {
      console.warn('⚠️ DynamoDB fetch failed, falling back to Firestore:', dynamoError)
      
      // Fallback to Firestore
      const ordersPath = `projects/${projectId}/orders`;
      ...
      orders = queryResult.docs.map(orderDoc => ({
        id: orderDoc.id || orderDoc.data()?.id,
        ...orderDoc.data()
      }))
    }
    ...
    const ordersWithStoreDetails = await Promise.all(
      orders.map(async (orderData) => {
        ...
        try {
          ...
          console.error('Error fetching store details for order:', orderData.id, storeError);
        }
...
// Get cancellation reason function name (raw ID) instead of formatted title
const getCancellationReasonName = (reasonId) => {
  if (!reasonId) return '';
  // Return the raw function name (ID) instead of formatted title
  return reasonId;
};
