...
const fetchStoreProducts = async (storeId) => {
  ...
  try {
    ...
    try {
      const { getProductsByStore } = await import('src/services/dynamoDBStoreProductsService')
      const productsData = await getProductsByStore(projectStore.selectedProject.id, storeId, { limit: 100 })
      console.log('✅ StoreDetails: Retrieved products from DynamoDB:', productsData.length)
