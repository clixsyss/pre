...
// Lazy: smartMirrorApp is null while SmartMirror is disabled. getStorage(null) throws at module
// eval time and crashes the app before the splash mounts. Resolve on first use instead.
...
export const deleteAd = async (projectId, adId) => {
  try {
    ...
    if (adDoc.exists()) {
      ...
      if (adData.imageUrl) {
        try {
          const imageRef = ref(getSmartStorage(), adData.imageUrl);
...
export const uploadAdImage = async (projectId, file) => {
  try {
    ...
    const storageRef = ref(getSmartStorage(), `projects/${projectId}/${fileName}`);
...
export const deleteAdImage = async (imagePath) => {
  try {
    const imageRef = ref(getSmartStorage(), imagePath);
    await deleteObject(imageRef);
  } catch (error) {
