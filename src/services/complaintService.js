      ...
      try {
        ...
        const complaint = {
          ...
          messages: [{
            ...
            timestamp: now, // Use timestamp for consistency
            imageUrl: complaintData.imageUrl || null,
            imageFileName: complaintData.imageFileName || null
          }],
          createdAt: now, // Number (timestamp) for DynamoDB GSI compatibility
          updatedAt: now, // Number (timestamp) for consistency
          lastMessageAt: now // Number (timestamp) for consistency
        };
  ...
  async addMessage(projectId, complaintId, messageData) {
    return performanceService.timeOperation('addMessage', async () => {
      try {
        ...
        const now = Date.now(); // Use timestamp (number) for DynamoDB compatibility
        const message = {
          id: Date.now().toString(),
  ...
  async updateComplaintStatus(projectId, complaintId, status, adminId = null) {
    return performanceService.timeOperation('updateComplaintStatus', async () => {
      try {
        ...
        const updateData = {
          status,
          updatedAt: Date.now() // Use timestamp (number) for DynamoDB compatibility
        };
