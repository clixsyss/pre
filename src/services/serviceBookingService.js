  ...
  async getUserServiceBookings(projectId, userId) {
    return performanceService.timeOperation('getUserServiceBookings', async () => {
      try {
        ...
        if (!projectId || !userId) {
          console.warn('ServiceBookingService: Missing projectId or userId')
          return []
        }
        
        // Use DynamoDB service first
        try {
          const bookings = await getDynamoDBUserServiceBookings(projectId, userId, { limit: 100 })
          
          // Sort by createdAt descending (DynamoDB doesn't guarantee order)
          bookings.sort((a, b) => {
          ...
          console.log('✅ User service bookings retrieved from DynamoDB:', { count: bookings.length })
          return bookings
        } catch (dynamoError) {
          console.warn('ServiceBookingService: DynamoDB fetch failed, falling back to Firestore:', dynamoError)
          ...
          console.log('✅ User service bookings retrieved from Firestore (fallback):', { count: bookings.length })
          return bookings
        }
      } catch (error) {
  ...
  async getAllServiceBookings(projectId) {
    return performanceService.timeOperation('getAllServiceBookings', async () => {
      try {
        ...
        if (!projectId) {
          console.warn('ServiceBookingService: Missing projectId')
          return []
        }
        
        // Use DynamoDB service first
        try {
          const bookings = await getServiceBookingsByProject(projectId, { limit: 100 })
          ...
          console.log('✅ All service bookings retrieved from DynamoDB:', { count: bookings.length })
          return bookings
        } catch (dynamoError) {
          console.warn('ServiceBookingService: DynamoDB fetch failed, falling back to Firestore:', dynamoError)
          // Fallback to Firestore
          ...
          console.log('✅ All service bookings retrieved from Firestore (fallback):', { count: bookings.length })
          return bookings
        }
      } catch (error) {
    ...
    return performanceService.timeOperation('getServiceBookingsByStatus', async () => {
      try {
        ...
        if (!projectId || !userId || !status) {
          console.warn('ServiceBookingService: Missing required parameters')
          return []
        }
        
        // Use DynamoDB service first
        try {
          const bookings = await getDynamoDBUserServiceBookings(projectId, userId, { 
            limit: 100,
            status: status
          })
          ...
          console.log(`✅ Service bookings with status '${status}' retrieved from DynamoDB:`, { count: bookings.length })
          return bookings
        } catch (dynamoError) {
          console.warn('ServiceBookingService: DynamoDB fetch failed, falling back to Firestore:', dynamoError)
          // Fallback to Firestore
          ...
          console.log('✅ Service bookings retrieved from Firestore (fallback):', result.docs.length)
  ...
  async getServiceBooking(projectId, bookingId) {
    return performanceService.timeOperation('getServiceBooking', async () => {
      try {
        ...
        // Use DynamoDB service first
        try {
          const booking = await getServiceBookingById(projectId, bookingId)
          
          if (booking) {
            console.log('✅ Service booking retrieved from DynamoDB:', booking.id)
            return booking
          } else {
          ...
          if (result.exists()) {
            ...
            console.log('✅ Service booking retrieved from Firestore (fallback):', booking.id)
            return booking
          } else {
            throw new Error('Service booking not found')
          }
