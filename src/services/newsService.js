...
class NewsService {
  ...
  async fetchNewsCategories(projectId, activeOnly = true) {
    try {
      ...
      try {
        const { getActiveNewsCategoriesByProject, getNewsCategoriesByProject } = await import('./dynamoDBNewsCategoriesService');
        
        const categories = activeOnly
          ? await getActiveNewsCategoriesByProject(projectId, { limit: 100 })
          : await getNewsCategoriesByProject(projectId, { limit: 100 });
        
        console.log('✅ News categories fetched from DynamoDB:', categories.length);
        return categories;
      } catch (dynamoError) {
        ...
        console.log('✅ News categories fetched from Firestore (fallback):', categories.length);
        return categories;
      }
    } catch (err) {
